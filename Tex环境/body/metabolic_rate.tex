...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figure/metabolic_rate/bmr_fitting.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figure/metabolic_rate/hr_fitting.png}
...
残差分析显示，BMR拟合残差在$\pm 20\%$以内波动，心率拟合残差在$\pm 15\%$以内波动，表明幂律模型对数据的拟合效果良好。
...
从图中可以看出，BMR和心率的观测标度指数与理论值高度接近，验证了血管分级模型的正确性。
...
从图中可以直观看出，体重越大的动物基础代谢率越高，但并非线性增长，而是遵循幂律关系。
...
\begin{enumerate}
    ...
    \item \textbf{模型对比}：幂律模型在R²、AIC、FPE和交叉验证中均表现最优，F检验表明幂律模型显著优于线性模型。
\end{enumerate}
