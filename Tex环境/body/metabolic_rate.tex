...
\par
本研究的技术路线如图\ref{fig:flowchart}所示。
...
\begin{itemize}
    ...
    \item \textbf{交叉验证R²（CV-R²）}：使用5折交叉验证评估模型泛化能力
\end{itemize}
...
\toprule[1.5pt]
模型 & R² & AIC & FPE & CV-R² & 参数数 \\
\midrule[1pt]
幂律模型 & 0.9994 & -0.64 & 0.62 & 0.4059 & 2 \\
二次模型 & 0.9992 & 3.72 & 1.03 & -4.3302 & 3 \\
线性模型 & 0.9948 & 12.62 & 5.61 & -20.7104 & 2 \\
指数模型 & 0.9723 & 22.68 & 30.00 & -89.8198 & 2 \\
对数模型 & 0.5619 & 39.25 & 474.94 & -10200.28 & 2 \\
\bottomrule[1.5pt]
\end{tabular}
\end{table}
...
模型 & R² & AIC & FPE & CV-R² & 参数数 \\
\midrule[1pt]
幂律模型 & 0.9800 & 43.75 & 1005.08 & -16.6486 & 2 \\
对数模型 & 0.9300 & 51.27 & 3519.21 & -1.5914 & 2 \\
指数模型 & 0.8434 & 56.10 & 7869.40 & -6.3488 & 2 \\
二次模型 & 0.6558 & 62.82 & 19457.60 & -4.5317 & 3 \\
线性模型 & 0.2572 & 65.44 & 37323.46 & -6.6451 & 2 \\
\bottomrule[1.5pt]
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{交叉验证表现最优}：CV-R²远高于其他模型，说明泛化能力更强；
...
图\ref{fig:bmr-fitting}展示了BMR和心率与体重的双对数拟合结果。

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\textwidth]{figure/metabolic_rate/bmr_hr_fitting.png}
\caption{BMR和心率与体重的幂律关系拟合}
\label{fig:bmr-fitting}
\end{figure}

从图中可以看出，实际数据点与拟合曲线高度吻合，Kleiber定律理论曲线也与数据趋势一致。
...
主要结论如下：
\begin{enumerate}
    \item 基于血管网络的分数维特性，可以从几何角度严格推导出基础代谢率与体重的3/4幂律关系，这一结果与West等人提出的代谢理论（MTE）相吻合\upcite{West1997}；
    \item 由速度守恒推导出血管半径衰减率$\alpha = m^{-1/2}$，由血容量守恒推导出长度增长率$\beta = m^{-1/3}$；
    \item 进一步推导出毛细血管总数$N_c \propto M^{3/4}$，从而得到BMR $\propto M^{3/4}$，这一结论支持了Kleiber定律的普适性\upcite{Niklas2015}；
    \item 主动脉长度$L_{\text{total}} \propto M^{1/4}$，脉搏传导时间$t \propto M^{1/4}$；
    \item 心率HR $\propto M^{-1/4}$，与BMR的推导形成自洽的理论体系，这一关系已在多种哺乳动物中得到验证\upcite{Mortola2023}；
    \item BMR拟合幂指数为0.7705，与理论值0.75仅偏差2.73\%，决定系数$R^2 = 0.999432$；
    \item 心率拟合幂指数为-0.2620，与理论值-0.25偏差4.79\%，决定系数$R^2 = 0.979997$；
    \item 真实数据与Kleiber定律高度吻合，验证了我们推论的正确性。
\end{enumerate}

本研究不仅从理论上支持了Kleiber定律，还通过实际数据为其提供了实证支持。正如Agutter和Wheatley所指出的，代谢标度关系的研究对于理解生命的基本规律具有重要意义\upcite{Agutter2004}，本研究的结果进一步巩固了Kleiber定律作为生物学基本规律的地位。

未来的研究可以进一步扩展到更多物种，包括植物和微生物，以验证这一幂律关系的普适性\upcite{Niklas2015}。此外，结合现代生物技术如代谢组学和基因组学数据，可以更深入地理解代谢率与体重关系背后的分子机制。
